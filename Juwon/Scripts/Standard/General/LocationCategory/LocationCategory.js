...
const $locationCategoryGrid = $(`#locationCategoryGrid`);

/** create */
const cLocationCategoryName = document.getElementById(`cLocationCategoryName`);
...
var locationCategoryId = 0;

async function Initialize() {
    await LocationCategoryGrid();
    await ReloadLocationCategoryGrid();
}
...
//LOCATIONCATEGORY GRID
async function LocationCategoryGrid() {
    "use strict";
    $locationCategoryGrid.jqGrid({
        colModel: [
            { name: "LocationCategoryId", label: "", key: true, hidden: true },
            { name: "LocationCategoryName", label: "Name", width: 100, align: 'center', search: false },
            {
        ...
        beforeRequest: function () {
            $(`#gbox_locationCategoryGrid`).block({
        ...
        loadComplete: function () {
            $(`#gbox_locationCategoryGrid`).unblock();
            let ids = $locationCategoryGrid.getDataIDs();
            for (let i of ids) {
                let row = $locationCategoryGrid.getRowData(i);
                if (row.Active === "NO") {
                    $locationCategoryGrid.setCell(i, 'LocationCategoryName', '', { 'background-color': '#ffcc99' }, '');
                }
            }
        },
        onSelectRow: function (rowid, status, e, iRow, iCol) {
            if (parseInt(rowid) == locationCategoryId) {
                locationCategoryId = 0;
            }
            else {
                locationCategoryId = parseInt(rowid);
            }
            let row = $locationCategoryGrid.getRowData(rowid);
            if (row.Active === "NO") {
...
async function ReloadLocationCategoryGrid() {
    return new Promise(resolve => {
        let keyWord = searchInput.value == null ? "" : searchInput.value;
        let requestUrl = ``;
        if (showDeleted.checked) {
            requestUrl = `/LocationCategory/SearchAll?keyWord=${keyWord}`;
        }
        else {
            requestUrl = `/LocationCategory/SearchActive?keyWord=${keyWord}`;
        }
        $.ajax({
            url: requestUrl,
            type: `GET`,
